...
bool sub_591d(register s8_t *st, s8_t *p2) {
    ...
    case DT_POINTER:
        /* For function pointers, check if both point to function types */
        /* Allow some flexibility for K&R vs prototype function pointer parsing differences */
        if (st->i_nextInfo && p2->i_nextInfo) {
            /* If one is ANODE (function type) and other is ENODE/SNODE, */
            /* check if they both represent function pointers */
            if ((st->i_nextInfo->c7 == ANODE || p2->i_nextInfo->c7 == ANODE) &&
                (st->i_nextInfo->dataType == DT_VOID || p2->i_nextInfo->dataType == DT_VOID ||
                 st->i_nextInfo->dataType == DT_INT || p2->i_nextInfo->dataType == DT_INT ||
                 st->i_nextInfo->dataType == DT_DOUBLE || p2->i_nextInfo->dataType == DT_DOUBLE)) {
                /* Accept function pointer with compatible return types */
                return true;
            }
        }
